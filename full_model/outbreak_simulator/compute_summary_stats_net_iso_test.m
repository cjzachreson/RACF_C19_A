...
data_dir = [root_dir, '\output_v10_OB_net_iso_test\facID_10000003_hyp\'];

output_dir = [root_dir, '\analysis\output_summary_stats_net_iso_test\'];
...
lockdown_compliance_OB = ["0.0"] %["0.9", "0.75", "0.5", "0.25", "0.0"];
...
%% plot probability of network isolation due to furlough
%% plot time to first detected case
for i = 1:size(testing_strategies, 2)
    strat_i = char(testing_strategies(i));
    data_dir_i = [data_dir, strat_i, '\'];
    ...
    for j = 1:size(lockdown_compliance, 2)
        ...
        network_iso_tot = output_linelist_ij.network_iso_tot(is_outbreak);   
        
        
        % output summary statistics. 
        network_iso_tot_q50 = quantile(network_iso_tot, 0.5);
        network_iso_tot_q90 = quantile(network_iso_tot, 0.9);
        
        scenario_label_ij = ...
            strcat(testing_strategies(i), '_LD_', lockdown_compliance(j));
        
        summary_stats_q50([scenario_label_ij], ["network_iso_tot_q50"]) = {network_iso_tot_q50};
        summary_stats_q90([scenario_label_ij], ["network_iso_tot_q90"]) = {network_iso_tot_q90};
        
        figure(i + size(testing_strategies, 2)*6)
        d = 1;
        h = histogram(network_iso_tot, 'BinEdges', [0:d:10],...
            'normalization', 'probability');
        xlabel('total person-days isolated due to furlough')
        hold on
        
        net_iso_tot_counts = (h.BinEdges(1:end-1))';
        frequency = (h.Values)';
        
        hist_table_net_iso = table(net_iso_tot_counts, frequency);
        
        hist_fname = ['net_iso_hist_',...
                      char(strrep(scenario_label_ij, '.', 'p')), '.csv'];
        
        writetable(hist_table_net_iso, [output_dir, hist_fname])
