...
function set_disease_params_default!(params::Disease_T, config::Setup_RACF.Config_T)
    ...
    # new code:
    params.beta = config.transmission_scaler
...
function set_disease_dict!(diseases::Diseases_T, config::Setup_RACF.Config_T)
    ...
    diseases.dict = Dict{String, Disease_T}() 

    diseases.names = ["Default"] #["Delta"] or ["H1N1"] or ["John Malkovich"]
...
function Efficacy_infection_Default(ln_neuts::Float64)::Float64
    ...
    eff = 1.0 / (1.0 + exp(-1.0 * k * (ln_neuts - c50)))
