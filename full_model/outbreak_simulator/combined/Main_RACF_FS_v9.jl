...
##### like the R0 script, but allows transmission to spread
# and records final size distribution: 
###### facility-specific R0 calibration: 
function run_Final_Size!(config::Setup_RACF.Config_T, 
    ...
    while n_instances < n_instances_tot
        ...
        t_i = 0.0
        t_f = 1000.0 # three months, maximum simulation time. 
    ...
    for im in immunity_states 
        ...
        for i in 1:1#n_populations #facility indices
            ...
            for tscale_j in transmission_scalers
                ...
                secondary_cases[it] = FS_est

            end

            output_main_FS = DataFrame(beta = transmission_scalers, 
                                       Final_Size = secondary_cases)

            output_fname = "$output_dir_L1\\beta_vs_Final_Size.csv"

            CSV.write(output_fname, output_main_FS)
