...
function setup_run_default!(config::Config_T, data_dirname::String)
    ...
    config.workers_G_fname = data_dirname * "\\workers_for_Julia_test.csv"
    config.workers_M_fname = data_dirname * "\\workers_M_for_Julia_test.csv" 
    #NOTE: in this version, the file workers_M_fname does not exist becauseee
    # we're not differentiating between general staff and medical staff.  
