...
function run_R0!(config::Setup_RACF.Config_T, 
    ...
    while n_instances < n_instances_tot
        ...
        # 2024 June 8th: adjusting for resident contact rates because I'm varying this for
        # sensitivity analysis: 
        index_case_id = Agents_RACF.select_random_agents_by_weight_lambda_res(agents, config, 1)
...
function main_R0()
    ...
    fac_list_fname = joinpath("$fac_list_dir", "hypothetical_facility_characteristics.csv")
    ...
    for im in immunity_states 
        ...
        for i in 3:3#1:1 #facility 1 has half as many staff as facility 3 in the original list 

            fac_i = i 
            fac_label = "facID_$(fac_list.service_id[fac_i])_hyp"


            for contact_rate_j in bkg_contact_rates

                contact_rate_label = replace("$contact_rate_j", "."=> "p")
                ...
                for tscale_j in transmission_scalers
                    ...
                    config_run.bkg_contact_rate_per_resident_per_day = contact_rate_j#3.0
                    ...
                    println("*****running R0 simulator for: lambda_res = $contact_rate_j, <beta_max> = $tscale_j*****")
