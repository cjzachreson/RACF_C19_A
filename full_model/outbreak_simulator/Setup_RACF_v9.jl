...
function setup_run_default!(config::Config_T, data_dirname::String)
    ...
    config_str = config_str*"\n"*"***testing probabilities*** \n"
        ...
        p_test_per_day = 0.0 # only used if testing is not sub-divided by agent type 
    ...
    config.residents_fname = joinpath(data_dirname, "residents_for_Julia_test.csv")
    config.workers_G_fname = joinpath(data_dirname, "workers_G_for_Julia_test.csv")
    config.workers_M_fname = joinpath(data_dirname, "workers_M_for_Julia_test.csv")
...
function write_config_details(config::Config_T, outdir::String)
    ...
    config_file = open(joinpath("$(outdir)", "config.txt"), "w")
