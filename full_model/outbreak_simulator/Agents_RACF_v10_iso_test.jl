...
function add_neighbours_from_N_list_v2!(a::Agent_T, 
    ...
    if haskey(id_to_contacts, id_i) 
        ...
        if is_resident(a)
            if isempty(contacts_i)
                network_iso_flag = true
                println("WARNING: resident $(a.id) isolated from staff members")
            end
        end
...
function assign_todays_contacts!(agents::Agents_T, 
    ...
    for (id_i, a) in agents.All # in updated version this will operate only on a subset
        network_iso = add_neighbours_from_N_list_v2!(a, day_of_week, id_to_contacts, id_i)
        if network_iso
            n_network_iso += 1
        end
    end

    return n_network_iso
