...
function run_OB!(config::Setup_RACF.Config_T, 
    ...
    while (n < n_tot)
        ...
        # total number of person-days for which residents 
        # are isolated due to staff furlough 
        n_network_iso_tot = 0 
        ...
        while !termination_flag
            ...
            if new_day 
                ...
                # June 6th, 2024 - checking for isolated residents, that is, residents
                # isolated not due to infection but due to all staff members assigned to them 
                # getting furloughed

                
                
                n_network_iso = Agents_RACF.assign_todays_contacts!(agents, 
                                                                   N_lists_new, 
                                                                   day_of_week) 

                n_network_iso_tot += n_network_iso
        ...
        if config.write_transmission_tree_flag && output_linelist.OB_declared[i]
            ...
            output_dir_run = joinpath("$output_dirname", "run_$(i)")
            if !ispath(output_dir_run)
                mkpath(output_dir_run)
            end

            CSV.write(joinpath("$(output_dir_run)", "$(ob_lab)detected_cases_$rand_label.csv"), all_detections)
            CSV.write(joinpath("$(output_dir_run)", "$(ob_lab)transmission_tree_$rand_label.csv"), all_transmissions)
        end
        ...
        push!(output_linelist.network_iso_tot, n_network_iso_tot)
...
function main_OB()
    ...
    test_configurations = String["asymp_testing_OB_only"]


    #lockdown_compliance = Float64[0.9, 0.75, 0.5, 0.25, 0.0]
    
    lockdown_compliance = Float64[0.9, 0.0]

    case_isolation_compliance = Float64[0.75, 0.5, 0.25]

    bkg_contact_rates = [3.0]#, 6.0, 10.0, 20.0, 30.0]

    R0_vals = [1.0, 1.5, 2.0, 2.4] #note, these will be captured +/- 0.1 (see calibration)

    # approximated from calibration of R0 and background contact rate 
    map_R0_bkgCR_to_beta = Dict(
        (1.0, 3.0) => 0.065,
        (1.0, 6.0) => 0.055,
        (1.0, 10.0) => 0.04,
        (1.0, 20.0) => 0.025,
    ...
    # list of facilities
    facility_list_dir = joinpath("$data_dir_L1", "$data_dir_L2")
    facility_list_fname = joinpath("$facility_list_dir", "hypothetical_facility_characteristics.csv")
    ...
    data_dirname = joinpath("$(data_dir_L1)", "$(data_dir_L2)", "$facility_label")#"$(data_dir_L1)$(data_dir_L2)\\$group_label\\$OB_label"
    ...
    for (TC_i) in test_configurations
        ...
        for R0 in R0_vals 
            ...
            # itereating through case isolation compliance
            for CI_compliance in case_isolation_compliance
 
                bkg_contact_rate = bkg_contact_rates[1] # 3.0

                tscale = map_R0_bkgCR_to_beta[(R0, bkg_contact_rate)]

                R0_str = replace("$R0", "."=> "p")
                R0_label = "R0_$R0_str"
                bkg_CR_str = replace("$bkg_contact_rate", "."=> "p")
                ...
                for LD_j in lockdown_compliance_i  
                    ...
                    # ensure lockdown compliance does not exceed case isolation compliance
                    if LD_j > CI_compliance
                        LD_compliance = CI_compliance
                    end


                    LD_str = replace("$LD_compliance", "."=> "p")
                    LD_label = "lockdown_compliance_$LD_str"

                    output_dir_L2 = joinpath("$test_config_label",
                                             "$R0_label",
                                             "$bkg_CR_label",
                                             "$tscale_label",
                                             "$CIC_label",
                    ...
                    #contact rate and transmission scalar:
                    config_run.transmission_scaler = tscale
                    config_run.bkg_contact_rate_per_resident_per_day = bkg_contact_rate

                    #distancing/lockdown parameters: 

                    config_run.resident_lockdown_efficacy = LD_compliance

                    config_run.resident_lockdown = true 
                    config_run.worker_case_isolation = true 
                    config_run.resident_case_isolation = true
                    config_run.resident_isolation_efficacy = CI_compliance#0.9 # imperfect compliance with resident case isolation 
