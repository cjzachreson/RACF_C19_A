...
function populate_workers_from_DataFrame_imDist_simple!(workers_out::Dict{Int64, Agent_T}, 
                                                        workers_DF::DataFrame, 
                                                        neut_dist::Distribution,
                                                        config::Setup_RACF.Config_T)#, N_lists)  
    ...
    for i = 1:n_workers
        ...
        workers_out[id_i].is_medical = false
        # parse room string (parse implementation)
        workers_out[id_i].roster = parse.(Int64, split(workers_DF.roster[i], ','))
...
function populate_residents_from_DataFrame_imDist_simple!(residents_out::Dict{Int64, Agent_T}, 
                                                          residents_DF::DataFrame, 
                                                          neut_dist::Distribution,
                                                          config::Setup_RACF.Config_T)#, N_lists)
    ...
    for i = 1:n_residents
        ...
        # needs
        residents_out[id_i].chc_needs = false
        residents_out[id_i].beh_needs = false
        residents_out[id_i].adl_needs = false
...
function reset_agent_states!(agents::Dict{Int64, Agent_T},
    ...
    # for debugging - check the vaccine efficacy: 
    #sum_efficacy = 0.0
    #n = 0

    #for (id, a) in agents
    #    sum_efficacy += a.immunity.protection_Infection["Default"]
    #    n += 1
    #end

    #println("mean efficacy: $(sum_efficacy/n)")
...
function compute_total_weight(a::Agent_T)::Float64
    ...
    w_tot = 0.0
    for (d, c) in a.contacts
        w_d = compute_total_weight_d(a, d)
        w_tot += w_d
    end
...
function sum_all_weights(A::Agents_T)::Float64

    w_TOT = 0.0

    for (id, a) in A.All

        w_TOT += compute_total_weight(a)
...
function compute_weight_fraction(A::Agents_T)::Tuple{Vector{Int64}, Vector{Float64}}

    id_vec = Vector{Int64}()
    weight_vec = Vector{Float64}()

    w_tot = sum_all_weights(A) 

    for (id, a) in A.All
        w_a = compute_total_weight(a)
        frac = w_a / w_tot
        push!(id_vec, id)
...
function select_random_agents_by_weight(A::Agents_T, config::Setup_RACF.Config_T, n::Int64)::Vector{Int64}
...
function test_weighted_sample(A::Agents_T, 
                              n_tot, 
                              config::Setup_RACF.Config_T)::Vector{Float64}

    id_vec, weight_vec = compute_weight_fraction(A)

    id_rand = sample(config.rng_infections, id_vec, Weights(weight_vec), n_tot)

    counts = countmap(id_rand)

    frac = Vector{Float64}()
    

    for id_i in eachindex(id_vec)

        id = id_vec[id_i]

        count_i = counts[id]

        push!(frac, count_i / n_tot)
    
    end

    ratios = frac ./ weight_vec
