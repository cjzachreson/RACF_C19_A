...
%     if ~isfield(worker_struct, 'roster')
%         'stop'
%     end
    ...
    %for rm_i = room_ids_rand %randomly iterate through rooms
     %for rm_i = room_ids % systematically iterate through rooms
        ...
        for i = 1:staff_per_room
            ...
            while isempty(selectable_ids)
                ...
                ids_min_workload = all_workers_today.id(all_workers_today.n_rooms == min_workload);
                
                % list of staff who have worked the room on previous days
                check_who_worked_before = @(x) is_room_there(x, rm_i, d);
                inds = cellfun(check_who_worked_before, all_workers_today.rooms, 'UniformOutput', false);
                ...
                if isempty(ids_worked_r_before)
                    ...
                    available_ids = all_ids_today(~ismember(all_ids_today, ids_working_r_today));
                else
                    available_ids = ids_worked_r_before(~ismember(ids_worked_r_before, ids_working_r_today));
                end
                
                %enforces workload constraint
                selectable_ids = available_ids(ismember(available_ids, ids_min_workload));
                ...
                if isempty(selectable_ids)
                    selectable_ids = ids_min_workload(~ismember(ids_min_workload, ids_working_r_today));
                end
            ...
            selected_worker = selectable_ids(randperm(numel(selectable_ids), 1)); % random selection
            %selected_worker = selectable_ids(1); % defined order
            ...
            %assign selected general staff member to room:
            if strcmp(staff_type, 'general')
                rooms(rm_i).workers_general = [rooms(rm_i).workers_general; [d, selected_worker, true]];
            elseif strcmp(staff_type, 'medical')
                rooms(rm_i).workers_medical = [rooms(rm_i).workers_medical; [d, selected_worker, true]];
            else
