...
function initialise_baseline_testing_schedule!(agents::Agents_T)
...
function assign_test_days!(a::Agent_T)
    r = a.roster
    ...
    test_day = sample(rng_testing, days_working)
    a.test_schedule[test_day] = 1
    else
        days_working = findall(==(1), a.roster)
        

        test_day_1 = sample(rng_testing, days_working)
...
function test_workers!(detections::DataFrame, p_test_per_day::Float64, 
                       agents::Agents_T, infected_agents::Dict{Int64, Float64},
...
function remove_worker_from_rooms!(a::Agent_T, rooms::Rooms_T, room_ids_to_update::Set{Int64})
...
function remove_workers!(agents::Agents_T, 
    ids_to_remove::Array{Int64, 1}, 
    N_list_TMG::N_list_temporal_multigraph_T, 
    ...
    for id in ids_to_remove
        ...
        N_list_d.id_to_contacts[a.id] = Array{Contact_T, 1}() # clear all contacts associated with removed worker (out edges).
...
function isolate_resident!(a::Agent_T, t_isolated::Float64, isolated_residents::Dict{Int64, Float64})
...
function isolate_residents!(agents::Agents_T, ids_to_isolate::Array{Int64, 1}, isolated_residents::Dict{Int64, Float64}, t::Float64)
...
function update_absentees!(agents::Agents_T, removal_period::Float64, removed_agents::Dict{Int64, Float64}, t::Float64, rooms::Rooms_T, room_ids_to_update::Set{Int64} )
...
function update_isolated_residents!(agents::Agents_T, removal_period::Float64, isolated_residents::Dict{Int64, Float64}, t::Float64)
...
function is_isolated(a::Agent_T)::Bool
    iso_flag = false 
    if a.t_removed >= 0.0
        iso_flag  = true
    end
